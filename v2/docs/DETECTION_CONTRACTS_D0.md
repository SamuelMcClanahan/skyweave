...
### D8.1 C3 opening (2026-08-20; restored 2026-08-23)

*Provenance: these four decisions were made 2026-08-20 in the planning
session and approved by Samuel; the entry was lost uncommitted during a
repository history rewrite. Campaign C-001 Shift 3/4 correctly flagged
the gap and declined to repair it. Restored verbatim by the planning
session 2026-08-23; C-001 Phase 1 work executed under the direct
implementation authorization in the campaign file and is not
retroactively altered by this restoration.*

| Item | Decision / record | Label |
| --- | --- | --- |
| Silicon (F-C1-4) | All five rig boards measure 128 MB of addressable DDR (bus-fault above 0x08000000, verified per board) — not the 256 MB variant the node design and the Phase A budget entries assumed. DECISION: proceed on 128 MB. The pool-aware budget model is hereby ratified: clip + daemon fixed residue are heap-bound (USABLE_HEAP_BYTES 28 MB, held below a live-measured ~35 MB MemAvailable); IVE detector state is CMA-bound (CMA_POOL_BYTES ~66 MB); the 160 MB pool-blind ceiling is retained as a third check; harness clip length = min(budget-room, heap-room). Detector footprint is five planes (~41 B/px) incl. the CCL staging buffer. Validated on hardware: measured CMA 30/53 MB matches arithmetic at 1152x648/1536x864; the 107 MB block at 2304x1296 fails allocation CLEANLY. This supersedes the Phase A closure's 160 MB-as-sole-ceiling row and the 12/78/171 clip defaults (now 36/18/unrealizable), both reasoned against 256 MB. 256 MB RV1106G3 procurement: Deferred, non-blocking. Supplier of the "Pico Pro Max" boards to be re-examined (remarked silicon; pattern matches the counterfeit SD04G card) | Chosen; board numbers Measured |
| C4 deployment resolution | 1152x648, Chosen by memory elimination: it is the only grid where detector CMA (30 MB, Measured) plus capture-path buffers fit the 66 MB pool on a camera-role board; 1536x864 fits only detector-without-camera; 2304x1296 is UNREALIZABLE on this silicon (detector alone exceeds the pool). D4 host quality holds at this grid (recall 0.996, false 0.000) and the D2 tripwire margin survives the 2x scale ratio. Sustained fps at this grid is OPEN until a clean sweep; nothing here asserts 30 fps | Chosen; fps Open |
| Centroid semantics (F-C1-6 BUG B) | The board's label-image scan assumed label = region-table index + 1, which the hardware does not guarantee once the area threshold compacts the table. DECISION: both board and host oracle compute the centroid as the first moment over the binary foreground mask restricted to the region's bbox (the SDK's canonical treatment of the label image as a mask). Overlapping-bbox ambiguity is declared and counted, never silent. BUG A (region loop bounded on count instead of the full non-contiguous astRegion array) is sanctioned for fix in the same rework, per the SDK samples. This resolves D0 divergence #2 as unimplementable-as-specified | Chosen |
| F-C1-6 remedy | DELIBERATELY OPEN. The CCL failure modes (threshold runaway; sub-cap failure) are diagnosed and instrumented, but no detector-config or clip change is sanctioned yet — Samuel directed further investigation before any tuning. warmup=30 is recorded as a mandatory driver correction (a convergence requirement, not tuning). Anti-tuning rule unchanged: no remedy may be derived from gate scenes | Open |

### C-002 closure decisions (2026-08-24)

| Item | Decision / record | Label |
| --- | --- | --- |
| Throughput ceiling | 1152x648 clean unpaced ceiling is 14.47 fps (three runs, spread 0.02%; A7 at 0.106 core; thermals flat) — the wall is the IVE engine/DMA path. 30 fps is NOT met at the only deployable resolution. Per the report's own rule this is a finding about the node, not a licence to relax the gate. DECISION (Samuel): open a throughput campaign (`campaigns/C-004-ive-throughput.md`) rather than accept the cadence now; the deployment cadence is Chosen at that campaign's end, from measurement | Measured; cadence Open |
| F-C2-1 detector nondeterminism | Component streams differ ~12% across identical runs (7,586/8,025/7,123 components). Severity judged NOT minor. Sanctioned firmware fix (BUG-class, like BUG A/B): zero-initialize the fg/bg/match planes at engine init and make the NULL factor-image argument explicit; then rerun the three-run determinism check. The E8 exact-counter rule is NOT amended before that rerun: if the fix restores exact agreement the rule stands; only if divergence persists is nondeterminism recorded as a Measured hardware property and the rule re-declared as bounded | Chosen; outcome Open |
| F-C2-6 thermal criterion | The declared 0.5 C / 5-minute plateau threshold sits below the sensor's ~0.594 C quantum and judged endpoints without noise immunity. Re-declared: window-MEAN rise with a threshold of 2 sensor quanta (1.2 C). The retained K1/K2 curves are re-judged under the re-declared criterion; no new runs. Recorded as a criterion-expressibility correction, not tuning: the original was declared before any board thermal number existed and is being replaced because the SENSOR cannot express it | Chosen |
| Report self-consistency | The generated report's preamble and section 1 still assert board-Pending/not-started above section 8's Measured C-002 rows. Sanctioned fix: the generator derives those claims from the same evidence that fills section 8; a consistency test asserts no PENDING claim coexists with Measured board rows | Recorded, fix Chosen |
| Rig reliability | Two Jetson outages and one switch-link death inside one campaign night (F-C2-4/5); the daemon outlived the infrastructure every time. The single-door topology makes the Jetson a SPOF for remote campaigns. HANDS: switch power/port/cable inspection, board LED check, card integrity verification on all boards (possible hard power loss), then the soak pair completes | Recorded |
